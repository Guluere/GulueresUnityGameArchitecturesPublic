 ...
 * 1.5V     -     : Added Rotation based on vector direction, accelarator 
 */
...
public class Sentient : SensorBaseTarget
{
    ...
    public Vector3 GetAllRotationalControlVector3But(VectorControlBehavioral Target, Dictionary<string, List<VectorControlBehavioral>> VectorControlBehavioralDictionary)
    {
        Vector3 result = Vector3.zero;
        ...
        foreach (List<VectorControlBehavioral> movementControl in VectorControlBehavioralDictionary.Values)
        {
            foreach (VectorControlBehavioral movementControl2 in movementControl)
            {
                if (movementControl2 == Target) break;
    ...
    public class AcceleratorVector : VectorControlBehavioral
    {
        public Vector3 TargetVelocity;

        public AcceleratorVector()
        {
            BehavioralName = "AcceleratorVector";
            Active = true;
            Strength = 1.0f;
        }
        ...
        public override Vector3 UpdatingMovementVector3()
        {
            Vector3 Change = (TargetVelocity - CurrentVector3).normalized * Strength * Time.deltaTime;

            //if(CurrentVector3.sqrMagnitude >= TargetVelocity.sqrMagnitude)
            //{
            //    CurrentVector3 = TargetVelocity;
            //}

            CurrentVector3 += Change;
    ...
    public class MoveTowardTarget : VectorControlBehavioral
    {
        ...
        public override Vector3 UpdatingMovementVector3()
        {
            ...
            MovingVector = new Vector3(MovingVector.x, MovingVector.y, MovingVector.z);
    ...
    public class SlowlyRotateTowardVector : VectorControlBehavioral
    {
        //Rotation speed will be equal to Vector3(1) * Strength
        Vector3 DefaultEular;
        public Vector3 Direction;
        Quaternion CurrentRot;

        public SlowlyRotateTowardVector(Vector3 defaultEular)
        {
            Active = true;
            Strength = 1;
            DefaultEular = defaultEular;
        }
        public override Vector3 UpdatingMovementVector3()
        {
            //if (Direction == Vector3.zero) Direction = Vector3.up;

            Vector3 _RotPrime = Quaternion.LookRotation(Direction, Vector3.up).eulerAngles;

            Vector3 _Rot = _RotPrime - CurrentRot.eulerAngles;

            //Debug.LogError(_Rot);

            Vector3 _RotInstanceNormalized = _Rot.normalized;
            Vector3 _RotInstance = _RotInstanceNormalized * Strength;

            float _t = 0;

            if (_RotInstance.sqrMagnitude >= _Rot.sqrMagnitude) _t = 1;
            else _t = Time.deltaTime * Strength; //_RotInstance.sqrMagnitude / _Rot.sqrMagnitude;

            CurrentRot = Quaternion.Lerp(CurrentRot, Quaternion.Euler(_RotPrime), _t);


            Direction = DefaultEular;
            //Debug.LogError(CurrentRot.eulerAngles);

            return CurrentRot.eulerAngles;
        }
    ...
    public class MaintainRotationOppositeOf : VectorControlBehavioral
    {
        Transform Parent;

        public MaintainRotationOppositeOf(Transform parent)
        {
            BehavioralName = "MaintainRotationOppositeOf";
            Active = true;
            Strength = 1.0f;
        ...
        public override Vector3 UpdatingMovementVector3()
        {
            //Vector3 LocalEuler = Owner.GetAllRotationalControlVector3But(this, Owner.AllRotationalControl);

            return -Parent.eulerAngles;
        }
    }

    public class OffsetRotationByVector : VectorControlBehavioral
    {
        public Vector3 OffsetVector;

        public OffsetRotationByVector()
        {
            BehavioralName = "OffsetRotationByVector";
            Active = true;
            Strength = 1.0f;
        }
